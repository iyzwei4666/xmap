package com.github.xmap.poi.mvp.ui.behavior;
...
public class AnchorBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private boolean mAnchorEnable=true;
    ...
    private SoftReference<View> mNestedScrollingChildRef;

    private SoftReference<RecyclerView> mRecycelerViewIn;
    ...
    public AnchorBottomSheetBehavior(Context context, AttributeSet attrs ) {
        ...
        a = context.obtainStyledAttributes(attrs, R.styleable.AnchorBottomSheetBehavior);
        if (attrs != null)
            mAnchorPoint = (int) a.getDimension( R.styleable.AnchorBottomSheetBehavior_anchorPoint, 0);
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection ) {
        ...
        mViewRef = new SoftReference<>(child);
        mNestedScrollingChildRef = new SoftReference<>( findScrollingChild( child ) );

        mRecycelerViewIn = new SoftReference<>( (RecyclerView) findRecyclerView( child ) );
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event ) {
        ...
        switch ( action ) {
            ...
            {
                if (mRecycelerViewIn == null )return false;
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed ) {
        ...
        if ( dy > 0 && (!isTouchOnRecyclerView || !mRecycelerViewIn.get().canScrollVertically(1))) { // Upward
            if ( newTop < mMinOffset ) {
                ...
                Log.i("BottomSheetBehavior" ,"393行调用setStateInternal");
                setStateInternal( STATE_EXPANDED );
            } else {
                consumed[1] = dy;
                ViewCompat.offsetTopAndBottom( child, -dy );
                Log.i("BottomSheetBehavior" ,"397行调用setStateInternal,-dy = "+(-dy));
            ...
            if ( ! ViewCompat.canScrollVertically(target, -1) ) {
                if ( newTop <= mMaxOffset || mHideable ) {
                    consumed[1] = dy;
                    ViewCompat.offsetTopAndBottom(child, -dy);
                    Log.i("BottomSheetBehavior" ,"406行调用setStateInternal");
                    setStateInternal(STATE_DRAGGING);
                } else {
                    consumed[1] = currentTop - mMaxOffset;
                    ViewCompat.offsetTopAndBottom(child, -consumed[1]);
                    Log.i("BottomSheetBehavior" ,"412行调用setStateInternal");
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target ) {
        if ( child.getTop() == mMinOffset ) {
            Log.i("BottomSheetBehavior" ,"423行调用setStateInternal");
            setStateInternal( STATE_EXPANDED );
        ...
        if ( mViewDragHelper.smoothSlideViewTo( child, child.getLeft(), top ) ) {
            Log.i("BottomSheetBehavior" ,"500行调用setStateInternal,smoothSlideViewTo的两个参数值是->"+child.getLeft()+"和"+top);
            setStateInternal( STATE_SETTLING );
            ViewCompat.postOnAnimation( child, new SettleRunnable( child, targetState ) );
        } else {
            Log.i("BottomSheetBehavior" ,"507行调用setStateInternal");
    ...
    public void setAnchorEnable(boolean enable){
        mAnchorEnable=enable;
    }
...
//            return;
        ...
        Log.i("BottomSheetBehavior" ,"628行调用setStateInternal");
        setStateInternal(STATE_SETTLING);
//        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
...
//        }
    ...
    private void setStateInternal(@State int state) {
        Log.i("BottomSheetBehavior" ,"现在的状态是：-->" + state );
        if (mState == state) {
//            return;
        }
        mState = state;
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewDragStateChanged( int state ) {
            if ( state == ViewDragHelper.STATE_DRAGGING ) {
                Log.i("BottomSheetBehavior" ,"750行调用setStateInternal");
                setStateInternal( STATE_DRAGGING );
            }
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel ) {
            ...
            if ( mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top) ) {
                Log.i("BottomSheetBehavior" ,"778行调用setStateInternal");
                setStateInternal(STATE_SETTLING);
                ViewCompat.postOnAnimation(releasedChild,
                        new SettleRunnable(releasedChild, targetState));
            } else {
                Log.i("BottomSheetBehavior" ,"788行调用setStateInternal");
                setStateInternal(targetState);
            }
        }
    ...
    private class SettleRunnable implements Runnable {
        ...
        public void run() {
            if ( mViewDragHelper != null  &&  mViewDragHelper.continueSettling( true ) ) {
                ...
                Log.i("BottomSheetBehavior" ,"844行调用setStateInternal");
                setStateInternal( mTargetState );
                mLastStableState = mTargetState ;
            }
        }
    }
